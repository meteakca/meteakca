...
A system-level benchmarking pipeline for the twelve graduate programs in my department, built from the University System of Georgia Board of Regents degrees-conferred extract. Each program is matched to its comparator set on CIP code and degree level, then rendered into a report with a peer table, a five-year trend, and a written interpretation — parameterized so a single source produces both a combined document and standalone per-program handouts.
